...
// Definitive anatomical layout with "fused" positioning
const LOBES = [
...
const ProceduralLobe = ({ data, audioData }: { data: any, audioData: AudioData }) => {
  ...
  useFrame((state) => {
    ...
    dtiRef.current?.position.lerp(targetPos, 0.12);
    ...
    if (meshRef.current) {
      ...
      mat.depthWrite = data.side === 'C';
    }

    if (dtiRef.current) {
      const mat = dtiRef.current.material as THREE.LineBasicMaterial;
      mat.opacity = THREE.MathUtils.lerp(mat.opacity, isPartActive ? 0.25 : 0.01 + intensity * 0.05, 0.1);
      mat.color.lerp(activeColor, 0.1);
    }

    const scale = isPartActive ? 1.1 : (1.0 + intensity * 0.05);
...
export const Brain = ({ audioData }: { audioData: AudioData }) => {
  return (
    <Center>
      <group scale={3.4} rotation={[0, -Math.PI / 2, 0]}>
        {LOBES.map((lobe, idx) => (
          <ProceduralLobe key={`${lobe.id}-${idx}`} data={lobe} audioData={audioData} />
        ))}
